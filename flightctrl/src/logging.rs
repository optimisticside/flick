...
///
/// See the BSP docs for more information on logging filters.
const FILTERS: &[bsp::usb::Filter] = &[
    // Try enabling this filter to only see log messages from main.rs.
    //
    // ("{{crate_name}}", None),
];

/// Initialize the USB logging system, and prepares the USB ISR with the poller
///
/// When `init` returns, the USB interrupt will be enabled, and the host may begin to interface the
/// device. You should only call this once.
///
/// # Panics
...
fn setup(poller: bsp::usb::Poller) {
    ...
    cortex_m::interrupt::free(|cs| {
        *POLLER.borrow(cs).borrow_mut() = Some(poller);
        // Safety: invoked in a critical section that also prepares the ISR shared memory. ISR
        // memory is ready by the time the ISR runs.
